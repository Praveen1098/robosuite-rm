...
class PickPlaceRM(ManipulationEnv):
    """
    ...
    Args:
        ...
        use_reward_machine (bool): Whether to use reward machine for rewards.
        
        rm_file (str, optional): Path to a reward machine file. If None, uses the default RM.
...
# Import reward machine components
from reward_machines.reward_machine import RewardMachine
from reward_machines.reward_functions import ConstantRewardFunction
...
class PickPlace(ManipulationEnv):
    ...
    def __init__(
        ...
        # reward machine settings
        self.use_reward_machine = use_reward_machine
...
class PickPlaceRewardFunction(RewardFunction):
    """
    Reward function class for the PickPlace environment that integrates with the reward_machines library.
    """
    ...
    def get_reward(self, s1, a, s2):
        """
        Get reward for transition from s1 to s2 with action a.
        ...
        Returns:
            float: Reward value
        """
        if self.env.reward_shaping:
            # Get the staged rewards
            staged_rewards = self.env.staged_rewards()
            reward = max(staged_rewards)
            # Scale reward if specified
            if self.env.reward_scale is not None:
                reward *= self.env.reward_scale
                if self.env.single_object_mode == 0:
                    reward /= 4.0
            return reward
        else:
            # Return 0 as intermediate reward in sparse case (rewards will come from RM transitions)
            return 0.0
...
class PickPlace(ManipulationEnv):
    ...
    def __init__(
        ...
        # Setup reward machine after environment initialization
        self._setup_reward_machine()

    def _setup_reward_machine(self):
        """
        Setup reward machine for the PickPlace task.
        
        The reward machine has the following states:
        - 0: Initial state (nothing achieved yet)
        - 1: Object is grasped
        - 2: Object is lifted
        - 3: Object is moved above bin
        - 4: Object is placed in bin (terminal state)
        """
        # Define reward function
        reward_function = PickPlaceRewardFunction(self)
        
        # Define reward machine
        self.reward_machine = RewardMachine()
        
        # Add states to the reward machine
        self.reward_machine.add_state(0)  # Initial state
        self.reward_machine.add_state(1)  # Object grasped
        self.reward_machine.add_state(2)  # Object lifted
        self.reward_machine.add_state(3)  # Object above bin
        self.reward_machine.add_state(4)  # Object in bin (terminal)
        
        # Set initial state
        self.reward_machine.set_initial_state(0)
        
        # Set terminal state
        self.reward_machine.add_terminal_state(4)
        
        # Add transitions
        # From initial to grasped
        self.reward_machine.add_transition(0, 1, self._is_grasped, 0.1, reward_function)
        
        # From grasped to lifted
        self.reward_machine.add_transition(1, 2, self._is_lifted, 0.15, reward_function)
        
        # From lifted to above bin
        self.reward_machine.add_transition(2, 3, self._is_above_bin, 0.2, reward_function)
        
        # From above bin to in bin
        self.reward_machine.add_transition(3, 4, self._is_in_bin, 1.0, reward_function)
        
        # Self-loops for all non-terminal states with small penalty to encourage progress
        for state in range(4):
            self.reward_machine.add_transition(state, state, lambda s1, a, s2, state=state: True, 0, reward_function)
        
        # Initialize current state of the reward machine
        self.u = self.reward_machine.get_initial_state()

    def _is_grasped(self, s1, a, s2):
        """Check if object is grasped."""
        # Identify active objects
        active_objs = []
        for i, obj in enumerate(self.objects):
            if self.single_object_mode == 0 or (self.single_object_mode in {1, 2} and i == self.object_id):
                active_objs.append(obj)
                
        # Check if any active object is grasped
        is_grasped = False
        for active_obj in active_objs:
            if self._check_grasp(
                gripper=self.robots[0].gripper,
                object_geoms=[g for g in active_obj.contact_geoms],
            ):
                is_grasped = True
                break
    ...
    def _is_lifted(self, s1, a, s2):
        """Check if object is lifted."""
        if not self._is_grasped(s1, a, s2):
            return False
        ...
        # Check if any active object is lifted
        is_lifted = False
        z_target = self.bin2_pos[2] + 0.05  # Lower threshold to detect lifting
        for active_obj in active_objs:
            obj_pos = self.sim.data.body_xpos[self.obj_body_id[active_obj.name]]
            if obj_pos[2] > z_target:
                is_lifted = True
                break
                
        return is_lifted

    def _is_above_bin(self, s1, a, s2):
        """Check if object is above bin."""
        if not self._is_lifted(s1, a, s2):
            return False
        ...
        # Check if any active object is above bin
        is_above_bin = False
        for i, active_obj in enumerate(active_objs):
            obj_pos = self.sim.data.body_xpos[self.obj_body_id[active_obj.name]]
            
            target_bin_id = i
            if self.single_object_mode in {1, 2}:
                target_bin_id = self.object_id
            ...
            if (bin_x_low < obj_pos[0] < bin_x_high and
                bin_y_low < obj_pos[1] < bin_y_high and
                obj_pos[2] > self.bin2_pos[2] + 0.1):
                is_above_bin = True
                break
                
        return is_above_bin

    def _is_in_bin(self, s1, a, s2):
        """Check if object is in bin."""
        # Update objects_in_bins array
        self._check_success()
        
        # Check if any active object is in its bin
        if self.single_object_mode in {1, 2}:
    ...
    def _get_current_state(self):
        """
        Get current state representation for the reward machine.
        This is a simple state representation for RM transitions.
        ...
        for i, obj in enumerate(self.objects):
            if self.single_object_mode == 0 or (self.single_object_mode in {1, 2} and i == self.object_id):
                state[f"obj_{i}_pos"] = self.sim.data.body_xpos[self.obj_body_id[obj.name]]
        
        # Gripper position
        for arm in self.robots[0].arms:
            state[f"gripper_{arm}_pos"] = self.sim.data.site_xpos[self.robots[0].eef_site_id[arm]]
        
        # Grasp state
        for i, obj in enumerate(self.objects):
            if self.single_object_mode == 0 or (self.single_object_mode in {1, 2} and i == self.object_id):
                state[f"obj_{i}_grasped"] = self._check_grasp(
                    gripper=self.robots[0].gripper,
                    object_geoms=[g for g in obj.contact_geoms],
                )
        
        # Success state
        state["objects_in_bins"] = self.objects_in_bins.copy()
    ...
    def reward(self, action=None):
        ...
        Returns:
            float: reward value
        """
        # Update the current state (used by reward machine transitions)
        self.previous_state = self.current_state
        self.current_state = self._get_current_state()
        
        # Get reward from reward machine
        if self.previous_state is not None:
            # Get the reward by executing a step in the reward machine
            reward, self.u = self.reward_machine.step(self.u, self.previous_state, action, self.current_state)
        else:
            reward = 0.0
            
        # Check if objects are in bins and add final reward if so
        self._check_success()
        
        # For sparse rewards without shaping, directly return success reward
        if not self.reward_shaping and np.sum(self.objects_in_bins) > 0:
            success_reward = np.sum(self.objects_in_bins)
            if self.reward_scale is not None:
                success_reward *= self.reward_scale
